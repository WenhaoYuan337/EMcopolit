...
class Up(nn.Module):
    def __init__(self, in_ch, out_ch, norm_layer, use_bias):
        ...
        self.up = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),  # 双线性插值上采样
...
class PixelDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, norm_layer=nn.BatchNorm2d, use_sigmoid=False):
        ...
        self.net = [
            nn.Conv2d(input_nc, ndf, kernel_size=1, stride=2, padding=0),
            nn.LeakyReLU(0.2, True),
            nn.Conv2d(ndf, ndf * 2, kernel_size=1, stride=2, padding=0, bias=use_bias),
            norm_layer(ndf * 2),
            nn.LeakyReLU(0.2, True),
            nn.Conv2d(ndf * 2, 1, kernel_size=1, stride=2, padding=0, bias=use_bias)]
